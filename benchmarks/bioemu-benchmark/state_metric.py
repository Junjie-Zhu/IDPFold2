...
def resample_with_noise(
    ...
    Args:
        x: Array of avg_rg to resample. Each row corresponds to a avg_rg point (shape [N x d]).
...
class DistributionMetrics2D:
    def __init__(
        ...
        This routine follows the overall procedure:
            ...
            1) Resample avg_rg and add Gaussian noise.
            2) Discretize resampled avg_rg and normalize to density.
    ...
    def score_nonzero(self, sample_projections: np.ndarray) -> tuple[float, float, float]:
        ...
        there are reference avg_rg points.
...
def compute_state_metrics(
    ...
    for test_case in tqdm(sample_projections):
